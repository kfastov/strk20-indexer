...
const { transfers, discovery, provingBlockId } = await makeTransfers(cfg, ks, account, provider);
try {
  ...
  const { callAndProof, details } = await buildAndProve(transfers, chain, provider, provingBlockId);
  ...
  if (dryRunStop(cfg, "the deposit is built, proved, screened and affordable")) { await discovery.close(); process.exit(0); }
