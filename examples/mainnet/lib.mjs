...
/**
 * Recompute the counterfactual address from first principles and cross-check it
 * against starknet.js's helper.
 *
 * This is the one irreversible mistake available here: STRK sent to an address
 * the class will not actually deploy to is gone. The node cannot be used as the
 * oracle — DEPLOY_ACCOUNT fee estimation with skipValidate accepts a wrong
 * address happily (verified) — so the check has to be local and independent.
 *
 * Formula (Starknet contract address):
 *   pedersen_array([ "STARKNET_CONTRACT_ADDRESS", deployer, salt, class_hash,
 *                    pedersen_array(constructor_calldata) ]) mod (2^251 - 256)
 * where pedersen_array chains the elements and then folds in the element count.
 * Validated against three accounts generated by sncast (an independent Rust
 * implementation) that are deployed on chain.
 */
export function computeCounterfactualAddress(salt, classHash, constructorCalldata, deployer = "0x0") {
  const ADDR_BOUND = 2n ** 251n - 256n;
  const PREFIX = "0x" + Buffer.from("STARKNET_CONTRACT_ADDRESS").toString("hex");
  const ctorHash = hash.computePedersenHashOnElements(constructorCalldata);
  const h = hash.computePedersenHashOnElements([PREFIX, deployer, salt, classHash, ctorHash]);
  return num.toHex(BigInt(h) % ADDR_BOUND);
}

export function assertAddressAgrees(address, salt, classHash, constructorCalldata) {
  const independent = computeCounterfactualAddress(salt, classHash, constructorCalldata);
  if (BigInt(independent) !== BigInt(address)) {
    fail(
      `ADDRESS DERIVATION MISMATCH — refusing to tell you to fund this address.\n` +
        `  starknet.js says      ${address}\n` +
        `  independent formula   ${independent}\n` +
        `  Do not send funds anywhere. This is a bug; report it.`
    );
  }
  return independent;
}
