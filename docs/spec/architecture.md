...
**R7 — Anchor placement: excluded from the content hash AND from the hashed bytes entirely.** All three judges mandate P1's anchor-exclusion rule. P1's mechanism ("hash computed with anchor:null") still leaves an anchor field inside the file whose value varies by cut time. Resolution (strictly stronger, simpler): the epoch file contains NO anchor field at all; the storage-proof anchor lives in `manifest.json` (summary) and a per-epoch sidecar file (full proof JSON), both outside content addressing (§4.3, §4.4). Late-backfilling mirrors outside the ~25–55k-block proof window produce byte-identical epoch files with an absent/`null` anchor in their manifest — a visible metadata gap, never a hash fork.
...
CREATE TABLE ingest_cursor (
  ...
  events_continuation TEXT                -- persisted getEvents continuation token
);
```
...
3. **Events-first scan**: `getEvents({from_block:{block_number:frontier+1}, to_block:{latest}, address:pool, chunk_size:1000, continuation_token})` with the token persisted after every page (crash-safe). Yields the pool-active block set (~0.23% of blocks) AND per-block event order = `event_index` assignment. `ImplementationReplaced` is an event, so upgrade blocks are always caught by this scan.
4. **Per active block** (bounded concurrency 8 for fetches; writes strictly sequential): `getStateUpdate({block_number})` → filter `state_diff.storage_diffs[pool]`, `replaced_classes`, `deployed_contracts`; `getBlockWithTxHashes({block_number})` → hash/parent/timestamp/status + `tx_hash → tx_index` from transaction position. One SQLite transaction per block: insert block, storage_log rows, events rows, class_history if applicable; advance `scan_frontier`.
...
a. **Pipeline liveness**: spawn `strk20 run --rpc-url http://fixture --feed-dir tmp --listen 127.0.0.1:0`; poll `/health` until OK and `latest_epoch = 1` (proves RPC→SQLite→cut→serve end-to-end).
...
h. **Compat + 409**: restart `strk20` with `--enable-compat`; raw `POST /v1/sync/incoming_state` with alice's key (reference body) → notes == O1 in reference JSON; response carries `X-Strk20-Mode: compat-keyed`; returned cursor round-trips into the client's persisted-cursor type (interop); request with forked `last_known_block` → HTTP 409 `BLOCK_REORGED`.
