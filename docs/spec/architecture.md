...
Mainnet defaults: primary `https://api.cartridge.gg/x/starknet/mainnet`, fallback `https://starknet.publicnode.com`. On September 7 the former Lava URL returned HTTP 410 with an endpoint-discontinued message. Cartridge served proofs at the observed head and head−5, refused head−100, and served the genesis-block header. Do not assume archive proofs are available: historical block access and proof retention are separate capabilities. Production can override the ordinary live RPC independently; it uses PublicNode for live reads and Cartridge for proof capability. Proof refusal retries are bounded and do not change the independently trusted checkpoint header. Every proof's block hash and global roots must bind to that header; disagreement remains a hard verification error. Alternative backfill source: `strk20 mirror pull <feed-url>` imports and checks another instance's public feed before switching to RPC at the feed head.
...
h. **Compat smoke**: restart `strk20` with `--enable-compat`; one raw `POST /v1/sync/incoming_state` with bob's real key (reference body) → success, response carries `X-Strk20-Mode: compat-keyed`, note set non-empty. That is all this leg claims, because a live server is the only thing that can claim it. The response payload is asserted here only as non-empty; the wire contract has cheaper carriers and is asserted there instead. `conformance.rs::compat_incoming_wire_equals_oracle` binds the same compat router to a loopback socket, POSTs the same request body this leg POSTs, and decodes the reply into `wire::IncomingSyncResponse`: notes == the O1 oracle across every page (so a truncated page or a wrong recipient fails), `block_ref` pinned to the head on each page, and the SERVER's cursor JSON parsed back into the client's persisted `DiscoveryCursor` — that last one is interop, distinct from `conformance.rs::cursor_reference_schema_round_trip`'s schema identity. Engine equivalence behind the handler is `conformance.rs::engine_over_sqlite_equals_engine_over_mock`, and a non-canonical `last_known_block` → HTTP 409 `BLOCK_REORGED` (plus the unknown-hash case, which must NOT 409) is `compat::tests::reorged_last_known_block_409s`. The key POSTed here is the real one on purpose: f(iii) scans the server's DB, feed and logs for it, and a fake key would make that scan vacuous. The process stays up for leg i's 503.
