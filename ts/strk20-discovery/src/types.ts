...
export interface ClientStatus {
   ...
   */
  engine: { kind: 'wasm' | 'mock'; label: string; provenance: string };
}

/** The upstream SDK socket (`createPrivateTransfers({ discoveryProvider })`). */
export interface DiscoveryProvider {
  getIncomingNotes(cursor?: string | null): Promise<{ notes: Note[]; cursor: string | null; complete: boolean }>;
  getOutgoingNotes(cursor?: string | null): Promise<{ notes: Note[]; cursor: string | null; complete: boolean }>;
  getTransactionHistory(opts?: { fromBlock?: number; limit?: number }): Promise<{
...
export interface DiscoveryClient {
   ...
   * key-derived value.
   */
  sync(opts?: { signal?: AbortSignal; onProgress?: (p: Progress) => void }): Promise<FeedState>;
