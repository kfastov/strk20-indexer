...
test("lost deployment response retains the precomputed hash and resumes without another send", async (t) => {
  // Deployment/receipt recovery must never call private discovery. Fail if it does.
  const unused = (): never => { throw new Error("Deployment must not use discovery"); };
  const discovery = { discoverNotes: unused, discoverChannels: unused,
    discoverRequirement: unused, atBlock: unused };
  let sends = 0,
    signatures = 0,
    signedHash = "";
