...
pub async fn verify_anchors(store: &FeedStore, transport: &dyn FeedTransport) -> Result<Value> {
...
///
/// The request names only the public pool and a public block, so it is
/// byte-identical for every user: keyless-compatible.
pub async fn ground_mirror_against_rpc(
    store: &FeedStore,
    ...
    for block in candidates {
        let result = match fetch_storage_proof(rpc, &pool, block).await {
            Ok(v) => v,
        ...
        let local =
            strk20_feed::mpt::storage_root(&store.full_slot_set_as_of(block)?);
        if local != chain_root {
    ...
    if mismatches.is_empty() {
        return Ok(Grounding::Unavailable(format!(
            "no candidate block was provable by {rpc}: {}",
            unavailable.join("; ")
        )));
    }
...
fn is_proof_unavailable(e: &anyhow::Error) -> bool {
    let msg = format!("{e:#}");
...
async fn fetch_storage_proof(rpc: &str, pool: &Felt, block: u64) -> Result<Value> {
    let http = reqwest::Client::builder()
        .user_agent(concat!("strk20-sync/", env!("CARGO_PKG_VERSION")))
        .build()?;
    // `[]` rather than `null` for the list params: some backends reject null
    // outright and every backend accepts the empty list (LIVE-7).
    let body = json!({
